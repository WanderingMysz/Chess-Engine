...
static inline int idx_from_char(char col, char row) {return col-'a' + (row-'1')*8;}
// Returns the Index (0-63) of a given board coordinate, e.g. (1,1)
static inline int idx_from_int(int col, int row) {return (col-1) + (row-1)*8;}

static inline int col_from_idx(int idx) {return idx % 8 + 1;}

static inline int row_from_idx(int idx) {return idx / 8 + 1;}
//!SECTION
